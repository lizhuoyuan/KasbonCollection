 ...
 * @returns {Promise.<*>}
 */
function requestUrl(url, method, body, successCallBack: func, errorCallBack: func) {
    if ('GET' === method) {
...
async function post(url, body, successCallBack, errorCallBack) {
    /**
     * 请求头
     * @type {{Accept: string, "Content-Type": string, uk: string}}
     */
    const headers = {
    ...
    try {
        ...
        console.log(`postUrl:${url} , headers:${JSON.stringify(headers)} , body:${JSON.stringify(body)}`);
        let responseJson = await response.json();
